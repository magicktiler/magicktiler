...
public class GoogleMapsTiler extends MagickTiler {
	...
	protected TilesetInfo convert(File image, TilesetInfo info) throws TilingException {
		...
		try {
			...
			String baseImageFileName = tilesetRootDir.getAbsolutePath()+"/base."+processor.getExtension();
			...
			for(int z=0;z<updatedInfo.getZoomLevels();z++) {
				...
				baseImageFileName = tilesetRootDir.getAbsolutePath()+File.separator+"base"+z+"."+processor.getExtension();
				...
				processor.crop(baseImageFileName, 
						tilesBaseFileName+"_"+"%d"+"."+processor.getExtension(), 
						tileWidth, 
						tileHeight);
				...
				for(int x=0,i=0;x<baseInfo.getNumberOfXTiles(0);x++) {
					for(int y=0;y<baseInfo.getNumberOfYTiles(0);y++,i++) {
						File fOld = new File(tilesBaseFileName+"_"+i+"."+processor.getExtension());
						File fNew = new File(tilesBaseFileName+"_"+y+"_"+x+"."+processor.getExtension());
