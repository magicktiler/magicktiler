...
public class TMSTest extends BaseTest {
  ...
  public void testTMSTiling() throws TilingException {
    Collection<String> expectedTopLevel = Arrays.asList(new String[] { "0", "1", "2", "3", "4", "5", "preview.html",
        "tilemapresource.xml" });
    ...
    for (int i = 0; i < info.getZoomLevels(); i++) {
      assertTrue(
          "Wrong directory structure in zoom level " + i,
          checkZoomLevel(info, new File(tilesetRoot, new Integer(i).toString()),
